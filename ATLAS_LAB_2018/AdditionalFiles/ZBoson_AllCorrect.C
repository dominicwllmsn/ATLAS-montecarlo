...
void mini::Book(std::string choice){
  	...
  	h_lep_type = new TH1F("type", TString::Format("%s: Lepton type",choice.c_str()) , 10, 10.5, 20.5);
   	h_lep_cone = new TH1F("ptcone", TString::Format("%s: Invariant Mass",choice.c_str()) , 100, 40000, 140000);
	
	h_lep_pt = new TH1F("pt", TString::Format("%s: Transverse Momentum",choice.c_str()) , 100, -1000, 100000);
...
void mini::FillHist(std::string choice){
	...
	TLorentzVector mu3(0., 0., 0., 0.);
	 ...
	 int lep_charge3 = 0;
	 float Z_cut = 60000;
	 float pt_limit = 24000;
	 float ptcone_limit = 5000;
	 float etcone_limit = 8000;
	 ...
	 if (charge_check == 0 && lep_type[0] == lepton_type && lep_type[1] == lepton_type && lep_n == 2 && lep_pt[0] > pt_limit && lep_pt[1] > pt_limit && lep_ptcone30[0] < ptcone_limit && lep_ptcone30[1] < ptcone_limit && lep_etcone20[0] < etcone_limit && lep_etcone20[1] < etcone_limit)
	 {
		 mll = (mu1 + mu2).M();
		 if (mll > Z_cut)
		 {
		 	//h_lep_cone->Fill(lep_pt[0], weight);
		 	//h_lep_cone->Fill(lep_pt[1], weight);
		 	h_lep_cone->Fill(mll, weight);
		 }
	 }
	 ...
	 if (lep_charge2 == -1 * lep_charge1 && lep_charge3 == lep_charge1 && lep_type[0] == lepton_type && lep_type[1] == lepton_type && lep_type[2] == lepton_type && lep_n == 3 && lep_pt[0] > pt_limit && lep_pt[1] > pt_limit && lep_ptcone30[0] < ptcone_limit && lep_ptcone30[1] < ptcone_limit && lep_etcone20[0] < etcone_limit && lep_etcone20[1] < etcone_limit)
	 {
		 mll = (mu1 + mu2).M();
		 if (mll > Z_cut)
		 {
	...
	if (lep_charge3 == -1 * lep_charge1 && lep_charge2 == lep_charge1 && lep_type[0] == lepton_type && lep_type[1] == lepton_type && lep_type[2] == lepton_type && lep_n == 3 && lep_pt[0] > pt_limit && lep_pt[2] > pt_limit && lep_ptcone30[0] < ptcone_limit && lep_ptcone30[2] < ptcone_limit && lep_etcone20[0] < etcone_limit && lep_etcone20[2] < etcone_limit)
	{
		mll = (mu1 + mu3).M();
		if (mll > Z_cut)
		 {
		 	//h_lep_cone->Fill(lep_pt[0], weight);
		 	//h_lep_cone->Fill(lep_pt[2], weight);
		 	h_lep_cone->Fill(mll, weight);
		 }
	}
...
void mini::Style(std::string choice){
	...
	h_lep_cone->SetLineColor(kBlue);
	h_lep_cone->GetXaxis()->SetTitle("Invariant Mass (MeV)"); // X-axis title
