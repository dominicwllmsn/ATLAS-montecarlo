...
void mini::Book(std::string choice){
	...
	h_lep_pt = new TH1F("pt", TString::Format("%s: Transverse Momentum",choice.c_str()) , 100, -500, 100000);
...
void mini::FillHist(std::string choice){
	 ...
	 if (lep_charge2 == -1 * lep_charge1 && lep_charge3 == lep_charge1 && lep_type[0] == lepton_type && lep_type[1] == lepton_type && lep_type[2] == lepton_type)
	 {
		 //mll = (mu1 + mu2).M();
		 // No restriction on invariant mass
		 h_lep_pt->Fill(lep_pt[0], weight);
	...
	if (lep_charge3 == -1 * lep_charge1 && lep_charge2 == lep_charge1 && lep_type[0] == lepton_type && lep_type[1] == lepton_type && lep_type[2] == lepton_type)
	{
		//mll = (mu1 + mu3).M();
		// No restriction on invariant mass
		h_lep_pt->Fill(lep_pt[0], weight);
		h_lep_pt->Fill(lep_pt[2], weight);
	}
...
void mini::Style(std::string choice){
	...
	h_lep_pt->GetXaxis()->SetTitle("pt (MeV)"); // X-axis title
