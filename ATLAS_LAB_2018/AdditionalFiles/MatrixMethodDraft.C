...
void mini::Book(std::string choice){
  	...
  	h_lep_type = new TH1F("type", TString::Format("%s: Trigger Lepton",choice.c_str()) , 10, -0.5, 5);
   	h_lep_cone = new TH1F("ptcone", TString::Format("%s: Invariant Mass",choice.c_str()) , 100, 40000, 200000);
	...
	TLorentzVector mu3(0., 0., 0., 0.);
	 ...
	 if (charge_check == 0 && lep_type[0] == lepton_type && lep_type[1] == lepton_type && lep_n == 2 && lep_pt[0] > pt_limit && lep_pt[1] > pt_limit && lep_ptcone30[0] < ptcone_limit && lep_ptcone30[1] < ptcone_limit && lep_etcone20[0] < etcone_limit && lep_etcone20[1] < etcone_limit)
	 {
		 ...
		 if (mll > Z_cut)
		 {
		 	...
		 	if (lep_trigMatched[0] == 1 && lep_trigMatched[1] != 1)
		 	{
		 		h_lep_type->Fill(lep_trigMatched[0], weight);
		 	}
		 	
		 	if (lep_trigMatched[1] == 1 && lep_trigMatched[0] != 1)
		 	{
		 		h_lep_type->Fill(lep_trigMatched[1], weight);
		 	}
		 }
	 }
...
void mini::Style(std::string choice){
	...
	h_lep_type->SetLineColor(kBlue);
	h_lep_type->GetXaxis()->SetTitle("Trigger Lepton"); // X-axis title
	h_lep_type->GetYaxis()->SetTitle("Number of Entries"); // Y-axis title
	h_lep_type->SetAxisColor(2,"x"); // Set colour of X-axis
