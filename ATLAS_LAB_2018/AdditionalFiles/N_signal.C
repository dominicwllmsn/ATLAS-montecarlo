...
void mini::Book(std::string choice){
	...
	h_lep_pt = new TH1F("pt", TString::Format("%s: Invariant Mass",choice.c_str()) , 100, -500, 100000);
...
void mini::FillHist(std::string choice){
	 ...
	 if (charge_check != 0 && lep_type[0] == lep_type[1] && lep_pt[0] > pt_limit && lep_pt[1] > pt_limit && lep_n == 2)
	 {
		 mll = (mu1 + mu2).M();
	 ...
	 if (lep_charge1 == lep_charge2 && lep_charge3 != lep_charge1 && // Charge condition
		 lep_type[0] == lep_type[1] && // Flavour condition
		 lep_pt[0] > pt_limit && lep_pt[1] > pt_limit && // pt condition
		 lep_ptcone30[0] < ptcone_limit && lep_ptcone30[1] < ptcone_limit && // ptcone condition
		 lep_etcone20[0] < etcone_limit && lep_etcone20[1] < etcone_limit &&
		 lep_n == 3)
	 {
		 mll = (mu1 + mu2).M();
	...
	if (lep_charge1 == lep_charge3 && lep_charge2 != lep_charge1 && // Charge condition
		lep_type[0] == lep_type[2] &&  // Flavour condition
		lep_pt[0] > pt_limit && lep_pt[2] > pt_limit && // pt condition
		lep_ptcone30[0] < ptcone_limit && lep_ptcone30[2] < ptcone_limit && // ptcone condition
		lep_etcone20[0] < etcone_limit && lep_etcone20[2] < etcone_limit &&
		lep_n == 3)
	{
		mll = (mu1 + mu3).M();
