...
void mini::Book(std::string choice){
	...
	h_lep_pt = new TH1F("lep_pt", TString::Format("%s: Transverse Mass",choice.c_str()) , 100, 0, 150000);
...
void mini::FillHist(std::string choice){
	 ...
	 float delta_phi;
         ...
         float E_ptcone30_limit = 2500;
         float E_etcone20_limit = 3500;
         float Mu_ptcone30_limit = 2500;
         float Mu_etcone20_limit = 2000;
         float met_et_limit = 30000;
         float pt_limit = 30000;
         float E_mt_limit = 50000;
         float Mu_mt_limit = 40000;
         ...
         for (size_t lep_i=0; lep_i<lep_n; lep_i++)  
         	{	
           		...
           		if (lep_n == 1 && lep_type[lep_i] == lepton_type && lep_ptcone30[lep_i] < E_ptcone30_limit && lep_etcone20[lep_i] < 				E_etcone20_limit && met_et > met_et_limit && lep_pt[lep_i] > pt_limit)
           		{
...
void mini::Style(std::string choice){
	...
	h_lep_pt->GetXaxis()->SetTitle(" Transverse Mass (MeV)"); // X-axis title
