...
void mini::Book(std::string choice){
  	...
  	h_lep_type = new TH1F("type", TString::Format("%s: Lepton type",choice.c_str()) , 10, 10.5, 20.5);
   	h_lep_cone = new TH1F("pt", TString::Format("%s: Transverse Energy Cone",choice.c_str()) , 100, -2000, 7000);
	
	h_lep_pt = new TH1F("pt", TString::Format("%s: Transverse Momentum",choice.c_str()) , 100, 0, 100000);
...
void mini::FillHist(std::string choice){
	 ...
	 TLorentzVector mu1(0.,0.,0.,0.);
         ...
         if (lep_n == 2) charge_check = 0;
         if (lep_n == 3) charge_check1 = 0;
         ...
         for (size_t lep_i=0; lep_i<lep_n; lep_i++)  
           	{	
           		...
           		{
           			// use charge check to ensure correct lepton pair
           		...
           		{
           			charge_check1 = charge_check1 + lep_charge[lep_i];
           			// Determine lepton with greatest transv. momentum
           			if (lep_pt[lep_i] > pt_max) {pt_max = lep_pt[lep_i]; pt_ind = lep_i;}
           		}
           	}
        if (charge_check == 0 && lep_type[0]==lepton_type && lep_type[1]==lepton_type) 
		 {                 
		 mll = (mu1 + mu2).M();
		 if (abs(mll - Z_mass) < Z_range)
			 {        
			 h_lep_pt->Fill(lep_pt[0],weight);
			 h_lep_pt->Fill(lep_pt[1],weight);
			 }
		 }
	if (abs(charge_check1) == 1 && lep_type[0]==lepton_type && lep_type[1]==lepton_type && lep_type[2]==lepton_type)
	 	{
	 	if (lep_charge[0] == lep_charge[1]) odd_ind = 2;
	 	if (lep_charge[0] == lep_charge[2]) odd_ind = 1;
	 	if (lep_charge[1] == lep_charge[2]) odd_ind = 0;
	 	
	 	mu1.SetPtEtaPhiE(lep_pt[pt_ind], lep_eta[pt_ind], lep_phi[pt_ind], lep_E[pt_ind]);
	 	mu2.SetPtEtaPhiE(lep_pt[odd_ind], lep_eta[odd_ind], lep_phi[odd_ind], lep_E[odd_ind]);
	 	
	 	mll = (mu1 + mu2).M();
	 	if (lep_charge[pt_ind] != lep_charge[odd_ind] && lep_type[pt_ind] == lep_type[odd_ind] && abs(mll - Z_mass) < Z_range) 
		 	{
		 	h_lep_pt->Fill(lep_pt[pt_ind],weight);
		 	h_lep_pt->Fill(lep_pt[odd_ind],weight);
		 	}
	 	}
