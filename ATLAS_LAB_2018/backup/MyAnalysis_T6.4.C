...
void mini::Book(std::string choice){
  	...
  	h_lep_type = new TH1F("type", TString::Format("%s: Lepton type",choice.c_str()) , 10, 10.5, 20.5);
   	h_lep_pt = new TH1F("pt", TString::Format("%s: Invariant Mass",choice.c_str()) , 600, 0, 120000);
...
void mini::FillHist(std::string choice){
	 ...
	 h_lep_n->Fill(lep_n,weight);
        ...
        if (charge_check == 0 && lep_type[0]==13 && lep_type[1]==13) 
		 {                         
	   	 mll = (mu1 + mu2).M();
		 h_lep_pt->Fill(mll,weight);
		 }
	if (abs(charge_check1) == 1 && lep_type[0]==13 && lep_type[1]==13 && lep_type[2]==13)
	 	{
