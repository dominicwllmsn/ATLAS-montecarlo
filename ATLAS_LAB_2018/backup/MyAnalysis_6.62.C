	 ...
	 TLorentzVector mu1(0.,0.,0.,0.);
        ...
        if (charge_check == 0 && lep_type[0]==lepton_type && lep_type[1]==lepton_type && lep_n==2) 
		 ...
		 if (abs(mll - Z_mass) < Z_range)
			 {        
			 h_lep_cone->Fill(lep_ptcone30[0],weight);
			 h_lep_cone->Fill(lep_ptcone30[1],weight);
			 }
		 }
	if (abs(charge_check1) == 1 && lep_type[0]==lepton_type && lep_type[1]==lepton_type && lep_type[2]==lepton_type && lep_n==3)
	 	{
	 	...
	 	if (lep_charge[pt_ind] != lep_charge[odd_ind] && lep_type[pt_ind] == lep_type[odd_ind] && abs(mll - Z_mass) < Z_range) 
		 	{
		 	h_lep_cone->Fill(lep_ptcone30[pt_ind],weight);
		 	h_lep_cone->Fill(lep_ptcone30[odd_ind],weight);
		 	}
	 	}
