...
void mini::Book(std::string choice){
  	...
  	h_lep_type = new TH1F("type", TString::Format("%s: Lepton type",choice.c_str()) , 10, 10.5, 20.5);
   	h_lep_cone = new TH1F("lep_cone", TString::Format("%s: Transverse Energy Cone",choice.c_str()) , 100, -2000, 7000);
	
	h_lep_pt = new TH1F("pt", TString::Format("%s: Transverse Momentum",choice.c_str()) , 100, 0, 100000);
	h_lep_pt2 = new TH1F("pt2", TString::Format("%s: Transverse Momentum",choice.c_str()) , 100, 0, 100000);
		...
		if (lep_type[lep_i] == 11)
		     {
			     if (lep_n == 1)
			     {
					 h_lep_pt2->Fill(lep_pt[0],weight*100);
			     }
			     if (lep_n == 2)
			     {
					 h_lep_pt2->Fill(lep_pt[0],weight*100);
					 h_lep_pt2->Fill(lep_pt[1],weight*100);
			     }
			     if (lep_n == 3)
			     {
			     		 h_lep_pt2->Fill(lep_pt[0],weight*100);
					 h_lep_pt2->Fill(lep_pt[1],weight*100);
					 h_lep_pt2->Fill(lep_pt[2],weight*100);
			     }
		}
	}
