...
void mini::FillHist(std::string choice){
	...
	if (abs(charge_check1) == 1 && lep_type[0]==lepton_type && lep_type[1]==lepton_type && lep_type[2]==lepton_type)
	 	...
	 	if (lep_charge[pt_ind] != lep_charge[odd_ind] && lep_type[pt_ind] == lep_type[odd_ind]) 
		 	{
		 	h_lep_cone->Fill(lep_etcone20[pt_ind],weight);
		 	h_lep_cone->Fill(lep_etcone20[odd_ind],weight);
		 	}
	 	}
