...
void mini::Book(std::string choice){
  	...
  	h_lep_type = new TH1F("type", TString::Format("%s: Lepton type",choice.c_str()) , 10, 10.5, 20.5);
   	h_lep_pt = new TH1F("pt", TString::Format("%s: Muon (+)",choice.c_str()) , 100, 0, 60000);
     	h_lep2_pt = new TH1F("pt", TString::Format("%s: Muon (-)",choice.c_str()) , 100, 0, 60000);
...
void mini::FillHist(std::string choice){
	 ...
	 int b_jet = 0;
	   ...
	   if (lep_pt[lep_i] > 25000 && exactlyOne == 0)
	   	{
	   	exactlyOne == 1
	   	goodLepton = lep_i;
	   	}
	   if (lep_pt[lep_i] > 25000 && exactlyOne == 1)
	   	{
	   	return False;
	   	}
   	 			...
   	 			if ((met_phi > 0 && lep_phi[goodLepton] > 0) || (met_phi < 0 && lep_phi[goodLepton] < 0))
		   		{
		   			delta_phi = abs(met_phi - lep_phi[goodLepton]);
		   		}
		   		if ((met_phi > 0 && lep_phi[goodLepton] < 0) || (met_phi < 0 && lep_phi[goodLepton] > 0))
		   		{	
		   			delta_phi = abs(met_phi - lep_phi[goodLepton]);
		   			if (delta_phi > M_PI)
		   			{
		   				delta_phi = abs(abs(met_phi - lep_phi[goodLepton]) - 2*M_PI);
		   			}
		   			
		   		}
		   		
		   		m_T = sqrt(2.*met_et*lep_pt[goodLepton]*(1. - cos(delta_phi)));
		   		
   	 for (size_t jet_i = 0; jet_i < jet_n; jet_i++)
	 {
		 jet_check = jet_check + 1;
		 if (abs(jet_MV1[jet_i] - 0.7) < 0.05)	{b_jet = 1}	 
	 }
	 
	 if (jet_check >= 4 && met_et > 30000 && (m_T + met_et) > 60000 && b_jet == 1)
	 	{
...
void mini::Style(std::string choice){
	...
	// Example2: Change some properties!
	h_lep2_pt->GetXaxis()->SetTitle("p_t (MeV)"); // X-axis title
	h_lep2_pt->GetYaxis()->SetTitle("Number of entries"); // Y-axis title
	h_lep2_pt->SetAxisColor(2,"x"); // Set colour of X-axis
	// etc... (See online ROOT documentation!)
